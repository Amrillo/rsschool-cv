...
### Example of my own code:
    ...
    ```var whatTimeIsIt = n=> {
        if(n ==0) {return "12:00"};
        let hour = Math.floor(n/30);
        let min =  Math.floor((n-30*hour)*2);
        const hourmin = time=> {
        ...
        return `${hourmin(hour) == "00"? "12": hourmin(hour)}:${hourmin(min)}`;
    };
   ```
---
...
+ *Skillbox - Frontend-developer course- ongoing* 
+ *Internship in Exzadel company - Front-end developer in 2021*
+ *Epam internship - JS/FE PRE-SCHOOL 2022 (JAVASCRIPT)* 
